...
namespace http_handler {
    ...
    public:
        ...
        void Tick(std::chrono::milliseconds time_delta) {
            ...
            this->players_.MoveAllDogs(std::chrono::duration<double>(time_delta).count() * CLOCKS_PER_SEC);

            for (int i = 0; i < this->game_.GetGameSessions().size(); ++i)
                this->game_.GetGameSessions()[i]->GenerateLoot(time_delta);

            for (int i = 0; i < this->game_.GetGameSessions().size(); ++i) {
                this->game_.GetGameSessions()[i]->CollectionItems(std::chrono::duration<double>(time_delta).count() * CLOCKS_PER_SEC);
                this->game_.GetGameSessions()[i]->LeaveItems(std::chrono::duration<double>(time_delta).count() * CLOCKS_PER_SEC);
            }
