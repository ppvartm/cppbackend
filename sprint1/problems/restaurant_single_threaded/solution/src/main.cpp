...
private:
    ...
    void OnRoasted(sys::error_code ec) {
        if (ec) logger_.LogMessage("Roast error: "s + ec.what());
        else {
    ...
    void MarinadeOnion() {
        ...
        marinade_timer_.async_wait([self = shared_from_this()](sys::error_code ec) {self->OnMarinade(ec); });
    }
    void OnMarinade(sys::error_code ec) {

        if (ec) logger_.LogMessage("Marinade onion error: "s + ec.what());
        else {
            logger_.LogMessage("Onion has been marinde."sv);
            onion_marinade_ = true;
        }
        CheckReadiness(ec);
    }
    ...
    [[nodiscard]] bool CanAddOnion() const {
        ...
        return hamburger_.IsCutletRoasted() && onion_marinade_ && !hamburger_.HasOnion();
    }
